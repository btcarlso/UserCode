...
# write the SLHA file 
gluinomasses = [375.0, 425.0, 475.0, 550.0, 625.0, 700.0, 775.0, 850.0, 925.0, 1000.0, 1075.0]
higgsinomasses = [200.0, 250.0, 300.0, 350.0, 400.0, 450.0, 500.0, 550.0, 600.0]

counter = 0
maxtowrite = 10
for mgl in gluinomasses:
  for mhi in higgsinomasses:
    if(mgl - mhi < 175.0):
      continue
    counter += 1
    mst = 0.5*(mgl + mhi)
    slhaname = "model1_gluino"+str(mgl)+"_higgsino"+str(mhi)+".slha"
    subprocess.call("cp slha_top.txt "+slhaname, shell=True)
    slhaout = open(slhaname, 'a')
    slhaout.write("      1000006    "+str(mst)+"  # m(stop)\n")
    slhaout.write("      1000021    "+str(mgl)+"  # m(gluino)\n")
