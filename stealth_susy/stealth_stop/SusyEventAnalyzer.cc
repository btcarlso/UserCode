...
SusyEventAnalyzer::Run()
{
    ...
    try{
        ...
        createHistogram("h_NgenLept","","Events","N_{gen}(lept)",3,-0.5,2.5);
        ...
        while(iEntry != processNEvents && event.getEntry(iEntry++) != 0){
            ...
            if(cutFlowGen){
                ...
                for(size_t i=0; i< event.genParticles.size(); i++){
                    ...
                    //if(TMath::Abs(event.genParticles.at(i).pdgId)==1000039) cout << "gravitino mass: " <<event.genParticles.at(i).momentum.M() << endl;
                    
                    if(TMath::Abs(event.genParticles.at(i).pdgId)==5){
                        double pt=event.genParticles.at(i).momentum.Perp();
                        double eta=fabs(event.genParticles.at(i).momentum.Eta());
                        if(pt>30 && eta<2.4) nB++;
                    }
                    /*
                    if(TMath::Abs(event.genParticles.at(i).pdgId)>=1000000 && TMath::Abs(event.genParticles.at(i).pdgId)<=1000024) {
                    ...
                    if(TMath::Abs(event.genParticles.at(i).pdgId)==11  || TMath::Abs(event.genParticles.at(i).pdgId)==13 || TMath::Abs(event.genParticles.at(i).pdgId)==15)  {
                        ...
                        if(TMath::Abs(event.genParticles.at(i).mother->pdgId)==24) NleptW++;
                        if(!EMudecay1 && (TMath::Abs(event.genParticles.at(i).pdgId)==11 && TMath::Abs(event.genParticles.at(i).pdgId)==13)){
                            EMudecay1=true;
                        }
                        if(EMudecay1 && (TMath::Abs(event.genParticles.at(i).pdgId)==11 && TMath::Abs(event.genParticles.at(i).pdgId)==13)) EMudecay2=true;
                    }
                    
                    // cout << "W 4-vector " << endl;
                ...
                //cout << "Nw: " << Nw << endl;
                hName["h_NgenLept"]->Fill(NgenLept);
                //cout << "NgenLept: " << NgenLept << endl << endl;
                //cout << "NleptW: " << NleptW << endl << endl;
                
                //if(NleptW>=1)hName["h_cutflow_table"]->Fill("#geq 1 Lept. W",1);
                //if(NleptW==2)hName["h_cutflow_table"]->Fill("2 Lept. W",1);
                
                //if(EMudecay1 && EMudecay2 )hName["h_cutflow_table"]->Fill("Gen #mu,e",1);
            }
            ...
            std::vector<susy::Muon const*> looseMuons_noIso;
            std::vector<susy::Muon const*> tightMuons_noIso;
            ...
            for (size_t iMuon(0); iMuon != muons.size(); ++iMuon) {
                ...
                bool tightCutNoIso=muon.isGlobalMuon() && normChi2Cut       && nValidMuonHitsCut &&
                nMatchedStationsCut && dxyCut            && dzCut             &&
                nValidPixelHitsCut  && nTrackerLayersCut;
                
                NLMuon_noIso++;
                if(tightCutNoIso)NTMuon_noIso++;
                looseMuons_noIso.push_back(& muon);
                if (looseCut) looseMuons.push_back(& muon);
                if (tightCut) tightMuons.push_back(& muon);
                if(tightCutNoIso)tightMuons_noIso.push_back(& muon);
            }
  ...
  /*
            std::vector<TLorentzVector> upsilonCollection;
            
            TLorentzVector p1;
            TLorentzVector p2;
            
            for(int i=0; i<looseMuons_noIso.size(); i++){
                p1=looseMuons_noIso.at(i)->momentum;
                for(int j=0; j<looseMuons_noIso.size(); j++){
                    if(i==j) continue;
                    if(i>j) continue;
                    p2=looseMuons_noIso.at(j)->momentum;
                    p1+=p2; 
                    upsilonCollection.push_back(p1);
                }
            }

            cout << "number of Upsilons in collection: " << upsilonCollection.size() << endl;
            for(int i=0; i<upsilonCollection.size(); i++){
                cout << "i: " << i << " M: "<< upsilonCollection.at(i).M() << " Pt: " << upsilonCollection.at(i).Perp() << endl;
                hName["h_Upsilonmass"]->Fill(upsilonCollection.at(i).M());
            }
*/
            ...
            if (looseMuons_noIso.size() >= 1 && looseMuons_noIso.at(0)->momentum.Pt() < muon1_ptCut) looseMuons_noIso.clear();
            if (tightMuons_noIso.size() >= 1 && tightMuons_noIso.at(0)->momentum.Pt() < muon1_ptCut) tightMuons_noIso.clear();
            ...
            if (looseMuons_noIso.size() >= 2) {
                for (size_t i = 0; i < looseMuons_noIso.size() - 1; i++) {
                    for (size_t j = i + 1; j < looseMuons_noIso.size(); j++) {
                        if (looseMuons_noIso.at(i)->bestTrack->charge == looseMuons_noIso.at(j)->bestTrack->charge &&
                            getDeltaR(looseMuons_noIso.at(i)->momentum, looseMuons_noIso.at(j)->momentum) < 0.02)
                            looseMuons_noIso.erase(looseMuons_noIso.begin() + j);
                    }
                }
            }
            
             for (size_t i = 0; i < tightMuons.size(); i++) hName["h_muonEta"]->Fill(tightMuons.at(i)->momentum.Eta());
            ...
            for(size_t iEle(0); iEle != electrons.size(); ++iEle) {
                ...
                bool looseCut_noIso = looseDEtaInCut && looseDPhiInCut && looseDzVtxCut && looseNMissingHitsCut;
                if(looseCut_noIso)NLElec_noIso++;
                ...
                if (looseCut_noIso) looseElectrons_noIso.push_back(& electron);
            ...
            int nJets_raw=0;
            int nJets_ptcut=0;
            int nJets_removeMuon=0;
            int nJets_removeElectron=0;
            int nJets_removePhoton=0;
            int nJets_cleaned=0;
            ...
            for (size_t iJ(0); iJ != pfJets.size(); ++iJ) {
                ...
                //increment counters
                nJets_raw++;
                if(jet.momentum.Pt()>jet_ptCut){
                    nJets_ptcut++;
                    if(!sameAsTightMuon) nJets_removeMuon++;
                    if(!sameAsTightElectron) nJets_removeElectron++;
                    if(!sameAsMediumPhoton) nJets_removePhoton++;
                }
   ...
   //         if (looseMuons.size() + looseElectrons.size() + loosePhotons.size() == 0) continue;
            ...
            for (size_t i = 0; i < looseElectrons.size(); i++) {
                bool match=false;
                for(int igen=0; igen<genElectrons.size(); igen++){
                    if(looseElectrons.at(i)->momentum.DeltaR(genElectrons[igen])<0.3) match=true;
                }
                
                if(!match)continue;
                float pt=looseElectrons.at(i)->momentum.Perp();
                float eta= fabs(looseElectrons.at(i)->momentum.Eta());
                
                if(eta<1.44)hName["h_pass_pt_barrel"]->Fill(pt);
            ...
            for (size_t i = 0; i < tightElectrons.size(); i++) {
                bool match=false;
                for(int igen=0; igen<genElectrons.size(); igen++){
                    if(tightElectrons.at(i)->momentum.DeltaR(genElectrons[igen])<0.3) match=true;
                }
                
                if(!match)continue;
                float pt=tightElectrons.at(i)->momentum.Perp();
                float eta= fabs(tightElectrons.at(i)->momentum.Eta());
                
                if(eta<1.44)hName["h_pass_tight_pt_barrel"]->Fill(pt);
            ...
            for (size_t i = 0; i < goodPfJets.size(); i++) {
                ...
                if(goodPfJets.at(i)->bTagDiscriminators[susy::kCSV] > 0.898) cout << " btag T: ";*/
            ...
            if(nB==1)hName["h_nLight_1b"]->Fill(Nlight);
            ...
            if(jet_e->size()>=4 && st>300){
                /*
                 ...
                 hName["h_cutflow_table"]->Fill("1 L#mu & 1Le, Iso<0.2",1);
                 
                 if(tightElectrons.size()==1 && tightMuons.size()==1){
                 hName["h_cutflow_table"]->Fill("1 T#mu & 1Te, Iso<0.12",1);
                 if(!(looseElectrons.size()>1 || looseMuons.size()>1)){
                 hName["h_cutflow_table"]->Fill("Remove additional loose leptons",1);
                 if(Nb==0){
                 hName["h_cutflow_table"]->Fill("0 b-tags",1);
                 }
                ...
                if(tightElectrons_noIso.size()==1){
                    hName["h_cutflow_table"]->Fill("1 e (no Iso)",1);
                        if(tightMuons_noIso.size()==1){
                            hName["h_cutflow_table"]->Fill("1 e(no Iso) && 1 mu(no Iso)",1);
                            if(tightElectrons.size()==1){
                                hName["h_cutflow_table"]->Fill("1 e(Iso) && 1 mu(no Iso)",1);
                                if(tightMuons.size()==1){
                                    hName["h_cutflow_table"]->Fill("1 e(Iso) && 1 mu(Iso)",1);
