...
for infile in files:
    ...
    if store == "eos":
        cp_command = "cp susyEvents.root "+"/eos/uscms/store/user/"+eos_area+"/"+area+"/"+baseOutdir+"/"+outfile
    elif store == "dcache":
        output_area = "srm://cmssrm.fnal.gov:8443/11/store/user/"+dcache_area+"/"+area+"/"+baseOutdir
        cp_command = '/opt/d-cache/srm/bin/srmcp -debug=true -2 "file:////${_CONDOR_SCRATCH_DIR}/susyEvents.root" "'+output_area+"/"+outfile+'"'
